...
class FarmView(AbstractGrid):
    ...
    def __init__(
        ...
        for row in range(len(tile_list)):
            for col in range(len(tile_list[row])):
                ...
                image_name = f'images/{IMAGES[tile]}'
        ...
        player_image_name = f'images/{IMAGES[DOWN]}'
    ...
    def redraw(
        ...
        player_image_name = f'images/{IMAGES[player_direction]}'
        player_image = get_image(player_image_name, self.get_cell_size(), self.image_cache)
        self.create_image(
...
class ItemView(tk.Frame):
    ...
    def update(self, amount: int, selected: bool = False) -> None:
        ...
        # Update the View class according to selection
...
class FarmGame:
    ...
    def __init__(self, master: tk.Tk, map_file: str) -> None:
        ...
        image_label = tk.Label(self.master, image=get_image('images/header.png',
        ...
        self.middle_frame = tk.Frame(self.master, width=INVENTORY_WIDTH + FARM_WIDTH, height=FARM_WIDTH)
        self.middle_frame.pack(side=tk.TOP, fill=tk.BOTH)

        # Farm View instances
        FarmView.map_file = self.farm_model.get_map()
        self.farm_view = FarmView(self.middle_frame, self.farm_model.get_dimensions(), (FARM_WIDTH, FARM_WIDTH))
        self.farm_view.pack(side=tk.LEFT, anchor=tk.W)

        # Item View Class
        self.items_list = []
        for items in ITEMS:
            self.item_view = ItemView(self.middle_frame,
                                      items,
        ...
        file_menu.add_command(label="Map Selection", command=self.load_map_file)
        menu_bar.add_cascade(label="File", menu=file_menu)
        self.master.config(menu=menu_bar)

    def load_map_file(self) -> None:
        """
        Command for loading the map
        """
        new_map_file = filedialog.askopenfilename(initialdir="maps",
                                                  title="Select map file",
                                                  filetypes=[("text file", "*.txt")])
        if new_map_file:
            self.farm_view.clear()  # clear the existing widget
            self.farm_model = FarmModel(new_map_file)
            self.farm_view.set_dimensions(self.farm_model.get_dimensions())  # set the dimensions for the new map
            self.redraw()
    ...
    def redraw(self) -> None:
        ...
        for index, item in enumerate(ITEMS):
            if item == selected_item:
                self.items_list[index].update(self.farm_model.get_player().get_inventory().get(selected_item, 0), selected=True)
            else:
    ...
    def handle_keypress(self, event: tk.Event) -> None:
        ...
        # Plant the selected item
            ...
            selected_item = player.get_selected_item()
            # Check the selected item and position to plant
...
def play_game(root: tk.Tk, map_file: str) -> None:
    """
    Constructs the controller for game.

    Parameters:
        root: Main Game Window Frame
